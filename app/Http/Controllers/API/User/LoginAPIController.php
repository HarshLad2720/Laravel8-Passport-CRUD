...
class LoginAPIController extends Controller
{
    ...
    public function refreshingTokens(Request $request)
    {
        ...
        if ($accessToken) {
            $loginUser = User::where('loginUId', $accessToken->user_id)->with(['role'])->firstorfail();

            // Update scopes for each user access tokens
            $getToken = User::getUserActiveToken($loginUser->loginUId, $oauthClient->id);
            if (is_null($getToken)) {
                return User::GetError('The refresh token is invalid.');
            }

            $getToken->scopes = $loginUser->role->permissions->pluck('name')->toArray();
            $getToken->save();
        }
